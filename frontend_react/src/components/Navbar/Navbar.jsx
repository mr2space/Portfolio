    ...
    <nav className="app__navbar">
      <div className="app__navbar-logo">
        <a href="/" className="app__navbar-logo">
          <img src={images.prince_logo} alt="logo" />
        </a>
      </div>
      <ul className="app__navbar-links hidden lg:flex">
        {["home", "work", "contact", "skills"].map((item) => (
          <li
            key={`link-${item}`}
            className="p-0 flex-col justify-center items-center flex"
          >
            <div className=" w-[5px] h-[5px] rounded-full mb-1" />
            <a
              href={`/#${item}`}
              className=" text-gray-700 flex-col uppercase font-medium transition-all hover:text-3-primary"
            >
              {item}
            </a>
          </li>
